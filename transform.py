...
def make_label(img_dir, save_gt_folder, phase):
    ...
    for filename in tqdm(os.listdir(os.path.join(img_dir, phase))):
        ...
        lp_number = provinces[int(txt_list[0])] + alphabets[int(txt_list[1])] + ''.join(
            [ads[int(x)] for x in txt_list[2:]])
...
img_dir = 'D:/train_data/CCPD2020/ccpd_green'  # 改成自己的路径
save_gt_folder = 'D:/train_data/CCPD2020/PPOCR'  # 改成自己的路径
